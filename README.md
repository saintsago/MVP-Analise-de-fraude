...
- **Mirror público (Google Drive, "qualquer pessoa com o link" pode acessar):**
  ...
  Ele é mantido exclusivamente para fins de execução acadêmica deste MVP.
- O notebook principal baixa os dados **automaticamente** via [`gdown`](https://github.com/wkentaro/gdown)
  na Seção 3.2, sem necessidade de montar um Google Drive pessoal nem fazer upload manual.
...
4. **Tempo estimado de execução:** o pipeline treina um `StackingClassifier` (XGBoost,
   LightGBM e CatBoost, com early stopping e validação cruzada de 5 folds) sobre
   ~472 mil transações e pode levar dezenas de minutos em CPU. Recomenda-se manter a aba
